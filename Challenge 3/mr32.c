...
int calcDistance(int adcValue)
{
   ...
   static float dist[]  = {8.0, 10.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0, 45.0, 50.0, 55.0, 60.0, 65.0, 70.0, 75.0, 80.0};
   int i, distance = 0;
   ...
   for(i=1; i < NR_CALIB_VALUES; i++)
   {
      ...
      {
         distance = (int)(((dist[i] - dist[i-1]) * (adcValue - calibValue[i])) / (calibValue[i] - calibValue[i-1]) + dist[i] + 0.5);
         break;
      }
   }
   return distance;
